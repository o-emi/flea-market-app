...
<div class="item-detail-form">
    <div class="item-detail-form__inner">
            ...
            <section class="item-detail-form__header">
                ...
                <div class="item-detail-form__actions">
                    <div class="item-detail-form__action-item">
                        ...
                        <img
                            src="{{ asset(auth()->user()->likes()->where('item_id', $item->id)->exists() ? 'images/logo/heart.logo.pink.png' : 'images/logo/heart.logo.gray.png') }}" class="item-detail-form__icon"
                            alt="いいね">
                                </button>
                            </form>
                        @endauth
                        ...
                        @guest
                            <img src="{{ asset('images/logo/heart.logo.gray.png') }}"
                            class="item-detail-form__icon"
                            alt="いいね">
                        @endguest
        ...
        <section class="item-detail-form__section">
            <h2 class="item-detail-form__section-title">コメント（{{ $item->comments->count() }}）
            </h2>
                ...
                @foreach ($item->comments as $comment)
                    <article class="item-detail-form__comment">
                        <div class="item-detail-form__comment-user">
                            <div class="item-detail-form__comment-avatar">
                            </div>
                            <span class="item-detail-form__comment-username">{{ $comment->user->name }}
                            </span>
                        </div>
                        <div class="item-detail-form__comment-body">{{ $comment->comment }}
                        </div>
                    </article>
                @endforeach
            </div>
            ...
            <div class="item-detail-form__comment-post">
                <h3 class="item-detail-form__sub-title">商品へのコメント</h3>
                    ...
                    @else
                        <p class="item-detail-form__login-notice">コメントを投稿するにはログインしてください
                        </p>
                    @endauth
            </div>
        </section>
